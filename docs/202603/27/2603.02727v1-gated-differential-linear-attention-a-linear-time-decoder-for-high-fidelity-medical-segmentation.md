...
tldr: 针对医学图像分割中Transformer计算开销大和线性注意力机制精度不足的问题，本文提出PVT-GDLA。该模型采用门控差分线性注意力（GDLA）机制，通过双路径差分消除噪声并增强上下文，结合轻量化门控和局部增强分支，在保持线性时间复杂度的同时实现了高精度的边界分割，在多种医学影像任务上达到SOTA性能。
selection_source: carryover_cache
motivation: 旨在解决传统Transformer计算复杂度高以及现有线性注意力机制在医学图像分割中存在的注意力稀释和训练不稳定问题。
method: 提出GDLA机制，通过两个互补子空间的差分计算消除噪声，并结合门控机制与深度卷积分支来增强非线性和局部特征提取。
result: 在CT、MRI、超声和皮肤镜等多个数据集上，PVT-GDLA以更低的计算量实现了优于主流CNN、Transformer及线性注意力模型的分割精度。
conclusion: PVT-GDLA为临床环境下的高效、高保真医学图像分割提供了一种兼顾计算效率与全局建模能力的实用方案。
---

## 摘要
医学图像分割要求模型在保持临床部署效率的同时，能够保留精细的解剖边界。虽然 Transformer 能够捕捉长程依赖，但面临二次方注意力计算成本和庞大数据需求的问题；而 CNN 虽然计算友好，但在全局推理方面表现欠佳。线性注意力提供了 $\mathcal{O}(N)$ 的缩放能力，但通常表现出训练不稳定和注意力稀释的问题，导致生成的映射图较为模糊。我们提出了 PVT-GDLA，这是一种以解码器为中心的 Transformer，能够在线性时间内恢复清晰的长程依赖。其核心是门控差分线性注意力（GDLA），它在互补的查询/键（query/key）子空间上计算两条核化注意力路径，并利用可学习的通道级尺度进行相减，以消除共模噪声并放大相关上下文。一个轻量级的、特定于头的门控机制注入了非线性和输入自适应稀疏性，从而缓解了注意力汇聚（attention sink）问题；同时，一个带有深度可分离卷积的并行局部令牌混合分支加强了相邻令牌间的交互，提高了边界保真度，且所有这些都保持了 $\mathcal{O}(N)$ 的复杂度和较低的参数开销。结合预训练的金字塔视觉 Transformer（PVT）编码器，PVT-GDLA 在相同的训练预算下，在 CT、MRI、超声和皮肤镜检查基准测试中均达到了最先进的准确率，其参数量与 CNN、Transformer、混合及线性注意力基准模型相当，但 FLOPs 更低。PVT-GDLA 为临床环境和其他资源受限场景下的快速、可扩展、高保真医学分割提供了一条切实可行的路径。

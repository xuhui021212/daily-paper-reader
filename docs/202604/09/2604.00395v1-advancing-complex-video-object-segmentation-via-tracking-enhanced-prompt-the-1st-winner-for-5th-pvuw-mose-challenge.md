...
tldr: 本研究针对复杂视频对象分割（VOS）中微小及语义主导目标难以识别的问题，提出了 TEP（跟踪增强提示）方法。针对现有 SOTA 模型 SAM3 在杂乱环境下理解力不足的缺陷，TEP 采用无需训练的策略，整合外部跟踪模型与多模态大语言模型生成增强提示，辅助模型精准定位与分割。该方法在 PVUW Challenge 2026 竞赛中夺冠，证明了其在提升复杂场景目标适应性方面的卓越性能。
source: arxiv
...
motivation: 现有最先进的 SAM3 模型在处理复杂视频中的微小目标和语义主导目标时，因缺乏对特定目标的深度理解而表现不佳。
method: 提出一种名为 TEP 的无需训练方法，利用外部跟踪模型和多模态大语言模型为分割模型提供增强的跟踪提示。
result: "该方法在 PVUW Challenge 2026 复杂视频对象分割赛道的测试集上取得了 56.91% 的成绩，荣获第一名。"
conclusion: 通过引入外部跟踪与语义增强提示，可以有效弥补现有分割模型在复杂环境下对特定目标理解能力的不足。
---

## 摘要
在复杂视频对象分割任务中，研究人员需要在杂乱的环境中跟踪并分割特定目标，这严峻地考验了方法的目标理解能力和环境适应能力。尽管目前的先进方案 SAM3 在常规目标上表现出无与伦比的分割性能和鲁棒性，但在微小目标和语义主导的目标上表现不佳。这一局限性的根本原因在于 SAM3 对这些特定目标类型的理解不足。为解决这一问题，我们提出了 TEP：通过跟踪增强提示推进复杂视频对象分割。作为一种无需训练的方法，TEP 利用外部跟踪模型和多模态大语言模型引入跟踪增强提示，从而缓解了 SAM3 在理解这些挑战性目标时面临的困难。我们的方法在 PVUW Challenge 2026：复杂视频对象分割赛道的测试集上获得了第一名（56.91%）。
